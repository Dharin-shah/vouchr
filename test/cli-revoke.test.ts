...
test('CLI revoke does not echo a token-shaped positional or unknown-flag secret (SEC-1)', async (t) => {
  const dbPath = await testDbUrl(t);
  const keyB64 = randomBytes(32).toString('base64');
  const secret = 'SYNTHETIC-CREDENTIAL-B-DO-NOT-ECHO';
  ...
  for (const args of [['revoke', secret, '--yes'], ['revoke', `--${secret}`, '--yes']]) {
    ...
    assert.ok(!(res.stderr + res.stdout).includes(secret), `must not echo the secret in ${args.join(' ')}`);
  }
  ...
  assert.ok(!(recognized.stderr + recognized.stdout).includes(secret), 'must not echo the secret');
  ...
  assert.ok(!(unknownCommand.stderr + unknownCommand.stdout).includes(secret), 'must not echo the secret');
});
