...
export interface EnvelopeProvider {
  /** Bounded callers supply `signal`; implementations should forward it to their network client. */
  wrapDataKey(dek: Buffer, signal?: AbortSignal): Promise<Buffer>;
  unwrapDataKey(wrapped: Buffer, signal?: AbortSignal): Promise<Buffer>;
}

/**
 * Slack resolves a multi-workspace installation before listener acknowledgement, so an external KMS
 * must not be allowed to hold that path forever. Keep the external-key portion below Slack's
 * three-second window. This bounds KMS work; the host must still budget PostgreSQL and scheduling.
 */
export const DEFAULT_ENVELOPE_TIMEOUT_MS = 2_000;
/** One process starts at most this many KMS operations that have not actually settled. */
export const DEFAULT_ENVELOPE_MAX_UNRESOLVED = 16;

/** Fixed, secret-free dependency errors generated by the bounded wrapper. */
export class EnvelopeTimeoutError extends Error {
  constructor() {
    super('vouchr: envelope key operation timed out');
    this.name = 'EnvelopeTimeoutError';
  }
}

export class EnvelopeOverloadedError extends Error {
  constructor() {
    super('vouchr: too many unresolved envelope key operations');
    this.name = 'EnvelopeOverloadedError';
  }
}

export class EnvelopeUnavailableError extends Error {
  constructor() {
    super('vouchr: envelope key service is unavailable');
...
export class EnvelopeRequiredError extends Error {
  constructor() {
    super('vouchr: stored secret is not envelope encrypted');
    this.name = 'EnvelopeRequiredError';
  }
}

export class EnvelopeConfigurationError extends Error {
  constructor() {
    super('vouchr: envelope provider configuration is invalid');
    this.name = 'EnvelopeConfigurationError';
  }
}
...
const boundedEnvelopeProviders = new WeakSet<EnvelopeProvider>();
const defaultBoundedEnvelopeProviders = new WeakMap<EnvelopeProvider, EnvelopeProvider>();

/**
 * Put one finite deadline and one unresolved-work ceiling around an operator-supplied envelope.
 *
 * A timeout releases the caller, but deliberately does NOT release the admission slot until the
 * underlying operation really settles. A provider that ignores AbortSignal can therefore consume at
 * most `maxUnresolved` slots instead of accumulating one abandoned promise per Slack event. Providers
 * should honor the signal to release their socket and slot promptly; the shipped AWS adapter does.
 *
 * Wrapping is idempotent. Default wrappers are also cached per provider, so multiple stores in one
 * process share the same unresolved-work ceiling rather than multiplying it.
 */
export function boundedEnvelopeProvider(
  provider: EnvelopeProvider,
  bounds: EnvelopeBounds = {},
): EnvelopeProvider {
  let wrapDataKey: EnvelopeProvider['wrapDataKey'];
  let unwrapDataKey: EnvelopeProvider['unwrapDataKey'];
  try {
    if ((typeof provider !== 'object' && typeof provider !== 'function') || provider === null) {
      throw new EnvelopeConfigurationError();
    }
    wrapDataKey = provider.wrapDataKey;
    unwrapDataKey = provider.unwrapDataKey;
    if (typeof wrapDataKey !== 'function' || typeof unwrapDataKey !== 'function') {
      throw new EnvelopeConfigurationError();
    }
  } catch {
    // Accessor/proxy failures are configuration-controlled and may contain operator secrets.
    throw new EnvelopeConfigurationError();
  }
  if (boundedEnvelopeProviders.has(provider)) return provider;
  const customBounds = bounds.timeoutMs !== undefined || bounds.maxUnresolved !== undefined;
  if (!customBounds) {
    const existing = defaultBoundedEnvelopeProviders.get(provider);
    if (existing) return existing;
  }

  const timeoutMs = bounds.timeoutMs ?? DEFAULT_ENVELOPE_TIMEOUT_MS;
  const maxUnresolved = bounds.maxUnresolved ?? DEFAULT_ENVELOPE_MAX_UNRESOLVED;
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs <= 0 || timeoutMs > MAX_TIMER_MS) {
    throw new Error('Envelope timeout must be a positive safe integer within the Node timer range');
  }
  if (!Number.isSafeInteger(maxUnresolved) || maxUnresolved <= 0) {
    throw new Error('Envelope unresolved-operation limit must be a positive safe integer');
  }
  ...
  const run = async (
    ...
    if (unresolved >= maxUnresolved) throw new EnvelopeOverloadedError();
    unresolved++;
    ...
    const timeoutError = new EnvelopeTimeoutError();
    let timedOut = false;
    ...
    const work = Promise.resolve().then(async () => {
      ...
      if (!Buffer.isBuffer(value)) throw new EnvelopeUnavailableError();
      return value;
    });
    // This tracks the REAL operation lifetime. Promise.race below may reject first, but a provider
    // that ignores cancellation keeps its slot until it eventually settles (or process restart).
    void work.then(
      (value) => {
        // If timeout/caller cancellation won the race, no consumer can scrub the returned plaintext
        // DEK. Erase it as soon as an uncooperative provider eventually resolves.
        if (abandoned) value.fill(0);
        releaseOperation();
      },
      releaseOperation,
    );
    ...
    try {
      ...
      // Provider errors are extension-controlled and may reflect key material. Never retain them as
      // a cause or copy their name/message into a public/Slack authorization error (SEC-1).
      throw new EnvelopeUnavailableError();
    } finally {
      if (!operationSettled) abandoned = true;
      clearTimeout(timer);
  ...
  const wrapped: EnvelopeProvider = {
    wrapDataKey: (dek, signal) => run(
      (boundedSignal) => wrapDataKey.call(provider, dek, boundedSignal),
      signal,
    ),
    unwrapDataKey: (wrappedKey, signal) => run(
      (boundedSignal) => unwrapDataKey.call(provider, wrappedKey, boundedSignal),
      signal,
    ),
  };
  boundedEnvelopeProviders.add(wrapped);
  if (!customBounds) defaultBoundedEnvelopeProviders.set(provider, wrapped);
  return wrapped;
}
...
export async function seal(plaintext: string, keys: MasterKeys, envelope?: EnvelopeProvider): Promise<Buffer> {
  ...
  try {
    ...
    // A fulfilled provider call is not necessarily a valid wrapped-key result. Reject impossible
    // lengths before persisting an envelope that can never be opened (and before Buffer's length
    // writer can surface an implementation-specific RangeError).
    if (wrapped.length < 1 || wrapped.length > 0xffff) {
      wrapped.fill(0);
      throw new EnvelopeUnavailableError();
    }
...
async function unwrapEnvelopeBlob(
  blob: Buffer,
  envelope: EnvelopeProvider,
  onUnwrap?: () => void,
  signal?: AbortSignal,
): Promise<string> {
  if (blob.length < 3) throw new EnvelopeUnavailableError();
  const dekLen = blob.readUInt16BE(1);
  const bodyOffset = 3 + dekLen;
  if (dekLen < 1 || bodyOffset > blob.length || blob.length - bodyOffset < 28) {
    throw new EnvelopeUnavailableError();
  }
  const wrapped = blob.subarray(3, bodyOffset);
  const body = blob.subarray(bodyOffset);
...
/** Decrypt only scheme-0x01 ciphertext. No direct/master-key fallback is possible. */
export async function openEnvelope(
  ...
  if (blob[0] !== SCHEME_ENVELOPE) throw new EnvelopeRequiredError();
  return unwrapEnvelopeBlob(blob, envelope, onUnwrap, signal);
}

/**
 * Decrypt a stored secret, dispatching on format so explicitly migrating callers can read old rows.
 *
...
export async function open(
  ...
  let envErr: unknown;
  try {
    return await unwrapEnvelopeBlob(blob, envelope, onUnwrap, signal);
  } catch (e) {
