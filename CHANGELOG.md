...
- **Deployment proof and release gate (#216, #217):** `test/docker-smoke.sh` now proves the two-replica shape on the image (schema-owner migrate vs DML-only replicas, read-only root + arbitrary uid, a rolling restart during which every request to the surviving replica succeeds, an in-flight request answered across SIGTERM, PostgreSQL outage fail-closed + recovery); `deploy/k8s.yaml` adds a `preStop` sleep so a terminating pod keeps accepting until endpoint removal catches up with SIGTERM; the runtime image carries `node` only (npm, npx, corepack and yarn are stripped); `release.yml` publishes only a tag contained in `main`, smokes + Trivy-scans the exact pushed digest before signing it, and verifies the npm `gitHead` and image revision label are the release commit; CI enforces a coverage floor (lines 95 / branches 87), packs on Node 22 + 24, runs a weekly higher-iteration property job, and proves a credential-shaped value under `test/**` fails this repo's Gitleaks scan (the blanket `test/` allowlist in `.gitleaks.toml` is replaced by one exact historical fixture value). `guides/DEPLOYMENT.md` gains an operator runbook.
...
- **Backchannel (CIBA-style) authorization for background agents (#296).** `POST /v1/authorization`
  ...
  exact action (the `/v1/fetch` target envelope plus a bounded plain `bindingMessage`) without
  executing it, and `GET /v1/authorization/{id}` polls the durable outcome
  (`pending → approved | denied | expired`; a spent grant or swept row is `404`). Every `/v1/fetch`
  gate runs first; the pending row is the same single-use, action-fingerprint-bound approval a later
  `/v1/fetch` spends, so the credential boundary is unchanged. The Bolt control plane delivers the
  Approve/Deny surface on its own bounded timer (`install({ authorizationDeliveryIntervalMs })`,
  default 15 s; `sweepExpired()` runs one pass too) through the existing `recoverBrokerDenial`
  bridge, rendering the agent's statement as plain text on the prompt. New exports:
  `BrokerAuthorizationRequest`, `BrokerAuthorizationResponse`, `AuthorizationStatus`,
